public class Main {
    ...
    private static void locationTests(PrimitiveTestFramework t) {
        ...
        t.assertEqual("Location a1 should be in file a", new Location('a', 1).file(), 'a');
        t.assertEqual("Location g3 should be in file g", new Location('g', 3).file(), 'g');
        t.assertEqual("Location a1 should be in rank 1", new Location('a', 1).rank(), 1);
        t.assertEqual("Location h8 should be in rank 8", new Location('h', 8).rank(), 8);
    ...
    private static void boardTests(PrimitiveTestFramework t) {
        ...
        t.assertEqual("Starting position d3", board.getPiece(new Location('d', 3)), Piece.Empty);
        t.assertEqual("Starting position b7", board.getPiece(new Location('b', 7)), Piece.BlackPawn);
